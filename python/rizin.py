...
def bin2hex(binstr):
	...
	"""
    hexstr = binascii.b2a_hex(binstr).lower()
    return hexstr
...
def write_history():
    ...
    for i in range(1, len(list)):
        ...
        if len(w) > 3:
            t = {}
            t["size"] = int(w[2].strip(), 10)
            t["addr"] = int(w[3].strip(), 16)
            # TODO moar nfo here
            ret.append(t)
    return ret
...
def flag_list(mask):
    ...
    for i in range(1, len(list)):
        ...
        if len(w) > 3:
            t = {}
            t["addr"] = int(w[1].strip(), 16)
            t["size"] = int(w[3].strip(), 10)
            t["name"] = w[4].strip()
            ret.append(t)
    return ret
...
def paste(num, addr=None):
    if addr == None:
        r.cmd("yy %d" % num)
    else:
        r.cmd("yy %d @ 0x%x" % (num, addr))
...
def dbg_backtrace():
    ...
    for i in range(1, len(list)):
        ...
        if len(w) > 3:
            t = {}
            t["addr"] = int(w[1].strip(), 16)
            t["framesz"] = int(w[2].strip(), 10)
            t["varsz"] = int(w[3].strip(), 10)
            ret.append(t)
    return ret
